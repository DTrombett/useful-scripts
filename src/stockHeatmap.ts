...
import { closeBrowser, closeContext, launch } from "./utils/browser.ts";
import { getUserChoice } from "./utils/options.ts";

const stockHeatmap = async () => {
	let browser: Awaitable<Browser> | undefined;
	let context: Awaitable<BrowserContext> | undefined;
	...
	try {
		...
		// Exit gracefully
		await page?.close();
		if (context) await closeContext(await context);
		if (browser) await closeBrowser(await browser);
	}
};
